...
<hr /></td>
	...
	$staffno = strip_tags($_POST['regno']);
	$ID = strip_tags($_POST['ID']);
	$usernm = strip_tags($_POST['usernm']);
	$password = strip_tags($_POST['password']);
	$rpassword = strip_tags($_POST['rpassword']);
	$realpass = strip_tags($_POST['password']);
	$submit = $_POST['submit'];
	
	$query = mysql_query("SELECT * FROM staff WHERE StaffNo='$staffno'");
	...
	if($submit)
	{
		...
		if($staffno&&$ID&&$usernm&&$password&&$rpassword )
		{
				
			if($staffno==$dbregno&&$ID==$dbID)
			{ 
				if($staffno)
				{
						//check for existance
				$query1 =mysql_query("SELECT * FROM stafflogin WHERE StaffNo='$staffno'");
				$numrows1 =mysql_num_rows($query1);
					if(mysql_num_rows($query1)!=0)
					{
						echo "The Registration number has already been registered";
					}
					else
					{	//check for existance
						if($usernm)
						{
							$query2 =mysql_query("SELECT * FROM stafflogin WHERE staffuser='$usernm'");
							$numrows2 =mysql_num_rows($query2);
							if(mysql_num_rows($query2)!=0)
							{
								echo "The Username already Exit";
							}
							else
							{
							if($password==$rpassword)
							{
								...
								{	
								//check ID len
								echo "Maximim Limit for ID	is 12";
								}
								else
								{	
								//Check username Length
									if(strlen($usernm)>25)
									{
										echo "Maximim Limit for username is 25";	
									}
									else
									{
										...
										$rpassword = md5($rpassword); 
											//Register the User!
											mysql_query("INSERT INTO stafflogin (ID,staffuser,password,StaffNo,realpass) 
											VALUES (Null,'$usernm','$password','$staffno','$realpass')");	
											mysql_query("Update staff SET reg=1 WHERE StaffNo='$staffno'");
...
<form action="staffreg.php" method="post">
  <table width="52%" border="0" cellspacing="4" cellpadding="3" class="altrowstable" id="alternatecolor">
    <tr>
      <td width="58%">Staff number</td>
      <td width="42%">
	  <?php 
$sql="Select StaffNo from staff WHERE reg != 1";
$q=mysql_query($sql);

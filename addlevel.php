...
<table width="1001" border="0" align="center" cellpadding="3" cellspacing="4">
  <tr>
	...
	if($submit)
	{
		if($categ)
		{
			$query = mysql_query("SELECT * FROM level where level='$categ'");
			$numrows = mysql_num_rows($query);
			if(mysql_num_rows($query)!=0)
					{
						echo "The Level '$categ' already Exist";
					}
					else
					{
			if (ereg("[A-Za-z]$", $categ))
			{		
		 mysql_query("INSERT INTO level (id,level) 
		 VALUES (NULL,'$categ')");	
		echo "Level '$categ' has been added";
		}
		else 
	...
	<form action="addlevel.php" method="post" name="form1" id="form1">
        <table border="0" cellpadding="3" cellspacing="4" class="altrowstable" id="alternatecolor">
          <tr>
            <td width="123" height="27">Level Name</td>
            <td width="362"><input type="text" name="level" id="textfield"  value="<?php echo $categ; ?>"/></td>
          </tr>
